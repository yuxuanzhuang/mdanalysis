...
class TestAtomnames(TestAtomAttr):
    values = np.array(
        ["O", "C", "CA", "N", "CB", "CG", "CD", "NA", "CL", "OW"], dtype=object
    )
    single_value = "Ca2"
    ...
    def test_missing_values(self, attr):
        assert_equal(
            attr.are_values_missing(self.values),
            np.array(
                [
                    False,
                    False,
    ...
    def test_missing_value_label(self):
        ...
        assert_equal(
            self.attrclass.are_values_missing(values),
...
class AggregationMixin(TestAtomAttr):
    ...
    def test_get_segments(self, attr):
        assert_equal(
            attr.get_segments(DummyGroup([1])),
...
class TestResidueAttr(TopologyAttrMixin):
    ...
    def test_get_residues(self, attr):
        assert_equal(
            attr.get_residues(DummyGroup([1, 2, 1, 3])),
            self.values[[1, 2, 1, 3]],
        )
    ...
    def test_set_residues_singular(self, attr):
        ...
        assert_equal(
            attr.get_residues(dg),
            np.array([self.single_value] * 3, dtype=self.values.dtype),
        )

    def test_set_residues_plural(self, attr):
        attr.set_residues(DummyGroup([3, 0, 1]), np.array([23, 504, 2]))
        assert_almost_equal(
            attr.get_residues(DummyGroup([3, 0, 1])), np.array([23, 504, 2])
        )
    ...
    def test_get_segments(self, attr):
        ...
        """
        assert_equal(
            attr.get_segments(DummyGroup([0, 1, 1])),
...
class TestResids(TestResidueAttr):
    ...
    def test_set_residues(self, attr):
        attr.set_residues(DummyGroup([3, 0, 1]), np.array([23, 504, 27]))
        assert_almost_equal(
            attr.get_residues(DummyGroup([3, 0, 1])), np.array([23, 504, 27])
        )
...
class TestStringInterning:
    ...
    def universe(self):
        u = mda.Universe.empty(
            n_atoms=10, n_residues=2, atom_resindex=[0] * 5 + [1] * 5
        )
